...
export const CommentForm: FC<CommentProps> = ({ comment = undefined }) => {
  ...
  const onSubmit = () => {
    issueDispatcher(
      isEdit
        ? instantUpdateComment(issue, executedBy, comment?._id ?? "", text)
        : instantCreateComment(issue, executedBy, text)
    );
  };
  return (
    <form
      ...
      <div className="bg-white px-4 py-5 sm:p-6 shadow-md">
        <div className="grid grid-cols-6 gap-2">
          ...
          <div className="col-span-6 mb-7">
            <ReactQuill style={{ height: 150 }} value={text} onChange={setText} />
          </div>
        </div>
