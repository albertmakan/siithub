...
import {
  type ChangeGithubAccount,
  changeGithubAccountBodySchema,
  changeGithubAccount,
  deleteGithubAccount,
} from "./user-github-actions";

type GithubUsernameFormProps = {
  githubAccount?: GithubAccount;
};

export const GithubUsernameForm: FC<GithubUsernameFormProps> = ({ githubAccount = undefined }) => {
  ...
  const {
    ...
    handleSubmit,
    formState: { errors },
  } = useZodValidatedFrom<GithubAccount>(changeGithubAccountBodySchema, githubAccount);

  const updateGithubAccount = useAction<ChangeGithubAccount>(changeGithubAccount(), {
    onSuccess: () => {
  ...
  const removeGithubAccount = useAction(deleteGithubAccount(), {
    onSuccess: () => {
      ...
      <div className="overflow-hidden shadow sm:rounded-md">
        ...
        <div className="grid grid-cols-6">
          <div className="col-span-3 bg-gray-50 px-4 py-3 text-left sm:px-6">
            {githubAccount ? (
              <Button onClick={() => removeGithubAccount(undefined)} type="button">
                Delete
              </Button>
            ) : (
              <></>
