...
import { Label, getRepositoryLabels } from "./labelActions";
...
export function useSearchLabels(repositoryId: Repository["_id"], name: string, dependencies: any[] = []) {
  const { data } = useQuery(
    [`labels_${repositoryId}`, name, ...dependencies],
    () => getRepositoryLabels(repositoryId, name),
    { enabled: dependencies.reduce((acc, d) => acc && !d, true) && !!repositoryId }
  );

  return { labels: data?.data as Label[] };
}
