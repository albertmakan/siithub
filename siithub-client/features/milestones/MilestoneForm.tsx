...
export const MilestoneForm: FC<{ existingMilestone?: Milestone }> = ({ existingMilestone }) => {
  ...
  const {
    register: createMilestoneForm,
    handleSubmit,
    formState: { errors },
  } = useZodValidatedFrom<CreateMilestone>(milestoneBodySchema, existingMilestone);
  ...
  const createMilestoneAction = useAction<CreateMilestone>(createMilestoneFor(_id), {
    onSuccess: () => {
  ...
  const updateMilestoneAction = useAction<UpdateMilestone>(updateMilestoneFor(_id, existingMilestone?.localId ?? 0), {
    onSuccess: () => {
